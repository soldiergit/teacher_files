layui.use(['form','layer','upload','laydate','util'],function(){
    ...
    //渲染复选框数据
    $.post("/teacher_files_war/biz/dept_findAll.action", function (data) {//--所属部门
        ...
        success: function (data) {
            if (data.code === 0) {
                ...
                form.val("userInfo", {//表单回显
                    ...
                    "deptHide": data.data.dept.deptId,  //中间变量--防止用户重复刷新
                    "unitHide": data.data.unitIds,  //中间变量--防止用户重复刷新
                    // "roleHide": data.data.roleIds,  //中间变量
    ...
    form.on("submit(addUser)",function(data){
        layer.confirm('确定要修改吗？', {icon: 3, title: '提示信息'}, function (index) {
            ...
            // 实际使用时的提交信息
            $.post("/teacher_files_war/biz/teacher_update.action",{
                ...
                roleIds : $(".roleIds").val(),//角色
                roleNames : $(".roleNames").val(),//角色名称
