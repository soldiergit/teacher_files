layui.use(['form','layer','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        ...
        url: '/teacher_files_war/menuChildren_findAll.action',//数据接口
    ...
    table.on('tool(infoList)', function (obj) {
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get("/teacher_files_war/menuChildren_delete.action",{
                    id : data.id  //参数
                },function(data){
                    if (data.code === 0){
                        layer.msg("删除成功");
    ...
    $(".delBtn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("/teacher_files_war/menuChildren_deleteBatch.action",{
