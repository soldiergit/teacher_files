layui.config({
    ...
    var tableIns = table.render({
        ...
        url: '/teacher_files_war/biz/teacherItem_findByDept.action?deptId='+deptId,//数据接口
        page : true,//开启分页
    ...
    $.post("/teacher_files_war/biz/teacher_findAllByDept.action?deptId="+deptId, function (data) {//--主持人
        $('#personId').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#personId').append(new Option(item.teacherName, "1,"+item.teacherId));
        });
        //重新渲染select
        form.render('select');
    });
    ...
    $.post("/teacher_files_war/biz/teacherItemCategory_findAll.action", function (data) {//--教师项目类别
        $('#itemCategory').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#itemCategory').append(new Option(item.title, "4,"+item.id));
        });
        //重新渲染select
        form.render('select');
    });
    $.post("/teacher_files_war/biz/teacherItemLevel_findAll.action", function (data) {//--教师项目级别
        $('#itemLevel').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#itemLevel').append(new Option(item.title, "5,"+item.id));
        });
        //重新渲染select
        form.render('select');
    });
    ...
    form.on("select(personId)", function (data) { reloadTable(data)});
    form.on("select(unitId)", function (data) { reloadTable(data)});
    form.on("select(itemType)", function (data) { reloadTable(data)});
    form.on("select(itemCategory)", function (data) { reloadTable(data)});
    form.on("select(itemLevel)", function (data) { reloadTable(data)});
